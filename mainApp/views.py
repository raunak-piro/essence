...
def confirmationPage(Request):
    ...
    if (username):
        try:
            ...
            message = "Yhank You to shop with us !! Your Order Has Benn Placed Successfully "
    ...
    if(Request.method=='POST'):
        ...
        if(user is not None):
            ...
            if(user.is_superuser):
                ...
                # subject = 'Account Login - Team Essence'
                # message = "You have Successfully Loged in to Your Account | Team Essence"
                # email_from = settings.EMAIL_HOST_USER
                # recipient_list = [buyer.email, ]
                # send_mail( subject, message, email_from, recipient_list )
    ...
    if(Request.method=="POST"):
        ...
        if(password==cpassword):
            try:
                ...
                buyer.save()
                subject = 'Account Created - Team Essence'
                message = "Thanks to Create an Account with US !!! Now you can buy our Products Team Essence "
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [buyer.email, ]
                send_mail( subject, message, email_from, recipient_list )
...
def forgetPasswordPage1(Request):
    if Request.method == 'POST':
        ...
        try:
            ...
            messages.error(Request,"Invalid Username1")
...
def forgetPasswordPage2(Request):
    ...
    if ((Request.method == 'POST') and username):
        ...
        try:
            ...
            messages.error(Request,"Invalid Username2")
        ...
        if Request.method == 'POST':
            ...
            if (resetUser and otp):
                ...
                if (int(otp) == buyer.otp):
                    ...
                    cpassword = Request.POST.get("cpassword")
                    if password != cpassword:
                        messages.error(Request,"Password and Confirm Password Does't Matches!!!!!")

                    else:
                        user = User.objects.get(username = resetUser)
                        user.set_password(password)
                        user.save()
                        subject = 'Password Reset - Team Essence'
                        message = "Your Password for Essence Login is Reset Successfully "
                        email_from = settings.EMAIL_HOST_USER
                        recipient_list = [buyer.email, ]
                        send_mail( subject, message, email_from, recipient_list )
                    ...
                    messages.error(Request,"Un-Authorized1!!!!")

            else:
                messages.error(Request,"Un-Authorized2!!!!")
